...
;~ 		$msg="ready"
;~ 		Return('ready')
	If(PixelGetColor(50, 960) == 0x37202C) Then
		$msg="crafting"
		Return('crafting')
	...
	ElseIf(PixelGetColor(1157, 848) == 0xFFFFFF) Then
		$msg="wait"
		Return('wait')
	ElseIf(PixelGetColor(1157, 848) == 0x7B6B64 Or PixelGetColor(1157, 848) == 0xD92E14 ) Then
		$msg="craft"
		Return('craft')
	ElseIf(PixelGetColor(598, 943) == 0xCBD2CF) Then
		$msg="blueprint"
		Return('blueprint')
	ElseIf(PixelGetColor(1150, 848) == 0xFFFFFF Or PixelGetColor(1150, 848) == 0x000000) Then
		$msg="wait"
		Return('wait')
	Else
	$msg= "Else#0x" & Hex(PixelGetColor(1157, 848),6)
	EndIf
EndFunc
...
Func buy()
	...
	$aCoord = PixelSearch(383, 191, 611, 399, 0xEFE56B)
		If Not @error Then
			ConsoleWrite("buy x and Y are: " & $aCoord[0] & "," & $aCoord[1] & @CRLF)
			Local $iCheckSum = PixelChecksum($aCoord[0], $aCoord[1], $aCoord[0], $aCoord[1]+20)
			ConsoleWrite("iCheckSum#" & $iCheckSum & @CRLF)
			if($iCheckSum == 2673814673 or $iCheckSum == 3999018776) Then
				MouseClick("Primary",$aCoord[0], $aCoord[1])
				Sleep(50)
...
While 1
;~ 	ConsoleWrite("LOG#0x" & Hex(PixelGetColor(50, 960),6) & @CRLF)
Switch location()
	Case "wait"
